...
class CarStateExt:
  ...
  def update_longitudinal_upgrade(self, ret: structs.CarState, can_parsers: dict[StrEnum, CANParser]) -> None:
    ...
    if self.CP.openpilotLongitudinalControl:
      ...
      stuck = ret.cruiseState.enabled and settled and acm_accel <= -3.8 and acm_accel == self.acm_accel_last
      self.acm_accel_last = acm_accel
      self.acm_rail_counter = self.acm_rail_counter + 1 if stuck else 0

      if self.acm_settle_cooldown > 0 and settled:
        self.acm_settle_cooldown -= 1

      if self.acm_rail_counter >= 400 and self.acm_settle_cooldown == 0:
        self.set_speed -= conversion
        self.acm_rail_counter = 0
        self.acm_settle_cooldown = 3000 # 30s
