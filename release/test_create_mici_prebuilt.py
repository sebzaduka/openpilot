...
class TestCommaPrebuiltContract(unittest.TestCase):
  ...
  def test_uses_provisioned_key_by_default(self):
    self.assertIn('if test "$#" -gt 1', self.script)
    self.assertIn('DEFAULT_SSH_KEY="${PREBUILT_GITHUB_SSH_KEY:-/data/ssh/prebuilt-deploy-key}"', self.script)
    self.assertIn('SSH_KEY="$DEFAULT_SSH_KEY"', self.script)
    self.assertIn('--local-only) SSH_KEY=""', self.script)
    self.assertIn('if test -n "$SSH_KEY"', self.script)

  def test_uses_persistent_prebuilt_identity(self):
    self.assertIn('DEFAULT_GIT_IDENTITY="${PREBUILT_GIT_IDENTITY:-/data/ssh/prebuilt-gitconfig}"', self.script)
  ...
  def test_publication_is_authenticated_and_race_safe(self):
    self.assertIn("IdentitiesOnly=yes", self.script)
    self.assertIn("StrictHostKeyChecking=yes", self.script)
    self.assertIn("UserKnownHostsFile=%q", self.script)
    self.assertIn("github.com ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIOMqqnkV", self.script)
