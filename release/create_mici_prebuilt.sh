...
if test "$PUBLISH" -eq 1; then
  ...
  PUBLISHED_SOURCE="$(git ls-remote --heads "$GIT_ORIGIN" "$SOURCE_REMOTE_REF" | awk '{print $1}')"
  if test "$PUBLISHED_SOURCE" != "$SOURCE_COMMIT"; then
    echo "Current HEAD does not match the published $SOURCE_BRANCH branch" >&2
    exit 1
  fi
...
# The publishing commands run from the temporary repository, so import the
# exact annotated tag object created above before attempting either remote update.
git -C "$OUTPUT_DIR" fetch --quiet "$ROOT" \
  "+refs/tags/$PREBUILT_TAG:refs/tags/$PREBUILT_TAG"
git branch -f "$TARGET_BRANCH" "$RELEASE_COMMIT"

if test "$PUBLISH" -eq 1; then
  git -C "$OUTPUT_DIR" remote add publish-origin "$GIT_ORIGIN"
  git -C "$OUTPUT_DIR" remote set-url --push publish-origin "$GIT_ORIGIN"
