...
def test_write_local_metadata_records_artifact_identity(monkeypatch, tmp_path: Path):
  spec = fetcher.MODEL_SPECS["small"]
  destination = tmp_path / "openpilot/selfdrive/modeld/models"
  destination.mkdir(parents=True)
  chunk = destination / f"{spec.canonical}.chunk01of01"
  chunk.write_bytes(b"locally compiled model")
  Path(get_manifest_path(str(destination / spec.canonical))).write_text("1\n")
  monkeypatch.setattr(fetcher, "onnx_sha256", lambda repo_root, relative_path: "a" * 64)
  monkeypatch.setattr(fetcher, "tinygrad_ref", lambda repo_root: "b" * 40)

  fetcher.write_local_metadata(tmp_path, "small")
