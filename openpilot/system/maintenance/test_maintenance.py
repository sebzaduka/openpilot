...
def test_model_artifact_eligibility_always_requires_big_model(tmp_path: Path, monkeypatch):
  def validate(repo_root, destination, targets, required_source=None):
    assert targets == ["small", "dm", "big"]
    assert required_source == "local"
...
def test_auto_prebuilt_eligibility_requires_exact_tracking_tip(tmp_path: Path, monkeypatch):
  ...
  build_marker.write_text(json.dumps({
    "schema": 1,
    "commit": head,
    "artifact_policy": "local-only",
    "models": [{"target": target, "valid": True} for target in ("small", "dm", "big")],
  }))
...
def test_successful_prebuilt_is_local_only(tmp_path: Path, monkeypatch):
  params = FakeParams()
  ...
  monkeypatch.setattr(subprocess, "check_output", lambda command, **kwargs: commands.append(command) or "")

  Maintenance(params, str(tmp_path)).start_prebuilt()

  assert commands == [["release/create_mici_prebuilt.sh", "--local-only"]]
