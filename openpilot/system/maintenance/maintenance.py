...
from openpilot.selfdrive.modeld.fetch_compiled_models import validate_compatible_targets
...
DEFAULT_GITHUB_TOKEN = Path(os.getenv("PREBUILT_GITHUB_TOKEN_FILE", "/data/prebuilt-publishing/github-token"))
...
def github_https_remote(remote: str) -> str:
  """Convert the checkout's GitHub SSH remote to an HTTPS token remote."""
  ...
  if remote.startswith("https://github.com/"):
    return remote
  # Baseline tests and offline installations can use a local bare remote. The
  # production origin is GitHub and is normalized above.
  return remote
...
def git_is_ancestor(ancestor: str, descendant: str, cwd: str = BASEDIR) -> bool:
  try:
...
def model_artifact_eligibility(cwd: str) -> Eligibility:
  model_dir = Path(cwd, "openpilot/selfdrive/modeld/models")
  targets = ["small", "dm"]
  if chestnut_present() or (model_dir / "big_driving_tinygrad.pkl.chunkmanifest").is_file():
    targets.append("big")
  result = validate_compatible_targets(Path(cwd), model_dir, targets)
  if not result["valid"]:
    failed = next(item for item in result["results"] if not item["valid"])
    return Eligibility(False, failed["detail"])
  return Eligibility(True, "model artifacts ready")


def auto_prebuilt_eligibility(cwd: str = BASEDIR, offroad: bool = True,
                              update_available: bool = False) -> Eligibility:
  if not offroad:
  ...
  if update_available:
    return Eligibility(False, "source update ready; reboot to install")
  current_branch = git(["branch", "--show-current"], cwd)
  if not current_branch.endswith("-src"):
  ...
  try:
    ...
    return Eligibility(False, "current commit has not completed compilation")
  try:
    tracking_tip = git(["rev-parse", "--verify", f"refs/remotes/origin/{current_branch}"], cwd)
  except subprocess.CalledProcessError:
    return Eligibility(False, f"origin does not advertise source branch {current_branch}")
  if tracking_tip != head:
    return Eligibility(False, "source update ready; reboot to install")
  try:
  ...
  return model_artifact_eligibility(cwd)
...
class Maintenance:
  ...
  def git_env(self) -> tuple[dict[str, str], str]:
    askpass = tempfile.NamedTemporaryFile(mode="w", prefix="github-askpass.", delete=False)
    askpass.write("\n".join((
      "#!/bin/sh",
      "case \"${1:-}\" in",
      "  *Username*) printf '%s\\n' x-access-token ;;",
    ...
    env = os.environ.copy()
    env["PREBUILT_TOKEN_FILE"] = str(DEFAULT_GITHUB_TOKEN)
    env["GIT_ASKPASS"] = askpass.name
    env["GIT_TERMINAL_PROMPT"] = "0"
    return env, askpass.name

  def verify_published_source(self) -> None:
    # Tokenless Auto Prebuilt is intentionally local-only and has no published
    # source contract to verify.
    if not DEFAULT_GITHUB_TOKEN.is_file():
      return

    env, askpass = self.git_env()
    try:
      branch = git(["branch", "--show-current"], self.cwd)
      head = git(["rev-parse", "HEAD"], self.cwd)
      origin = github_https_remote(git(["remote", "get-url", "--push", "origin"], self.cwd))
      published = git(["ls-remote", "--heads", origin, f"refs/heads/{branch}"], self.cwd, env)
    except subprocess.CalledProcessError as e:
      detail = (e.output or "").strip()
      suffix = f": {detail}" if detail else ""
      raise RuntimeError(f"unable to verify source branch on published origin{suffix}") from e
    finally:
      os.unlink(askpass)

    fields = published.split()
    published_tip = fields[0] if len(fields) == 2 else ""
    if published_tip != head:
      raise RuntimeError("source branch changed on origin; reboot to install the latest source update and rebuild before Auto Prebuilt")

  def start_prebuilt(self) -> None:
    eligibility = auto_prebuilt_eligibility(self.cwd, self.params.get_bool("IsOffroad"),
                                            self.params.get_bool("UpdateAvailable"))
    if not eligibility.ready:
      raise RuntimeError(eligibility.reason)
    self.verify_published_source()
  ...
  def check_baseline(self) -> None:
    ready = self.params.get_bool("IsOffroad") and DEFAULT_GITHUB_TOKEN.is_file()
    ...
    available = tip != config["sha"] and not git_is_ancestor(tip, "HEAD", self.cwd)
  ...
  def start_baseline(self) -> None:
    if not self.params.get_bool("IsOffroad") or not DEFAULT_GITHUB_TOKEN.is_file():
  ...
  while True:
    try:
      ...
      eligibility = auto_prebuilt_eligibility(BASEDIR, params.get_bool("IsOffroad"),
                                              params.get_bool("UpdateAvailable"))
