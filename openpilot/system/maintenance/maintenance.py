...
def model_artifact_eligibility(cwd: str) -> Eligibility:
  ...
  targets = ["small", "dm", "big"]
  result = validate_compatible_targets(Path(cwd), model_dir, targets, required_source="local")
  if not result["valid"]:
...
def auto_prebuilt_eligibility(cwd: str = BASEDIR, offroad: bool = True,
  ...
  try:
    marker = json.loads(BUILD_MARKER.read_text())
    if marker.get("commit") != head:
      return Eligibility(False, "current commit has not completed compilation")
  except (FileNotFoundError, json.JSONDecodeError, OSError):
    return Eligibility(False, "current commit has not completed compilation")
  if marker.get("schema") != 1 or marker.get("artifact_policy") != "local-only" or not isinstance(marker.get("models"), list):
    return Eligibility(False, "current commit requires a new local-only device build")
  model_results = {result.get("target"): result for result in marker["models"] if isinstance(result, dict)}
  for target in ("small", "dm", "big"):
    if not model_results.get(target, {}).get("valid"):
      return Eligibility(False, f"current build did not produce a compatible {target} model artifact")
  try:
...
class Maintenance:
  ...
  def start_prebuilt(self) -> None:
    ...
    try:
      subprocess.check_output(["release/create_mici_prebuilt.sh", "--local-only"], cwd=self.cwd, env=env,
